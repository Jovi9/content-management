<x-app-layout>
    @if (isset($is_SubMenu))
        @section('title', 'Create ' . ucwords($subMenu) . ' Contents')
    @else
        @section('title', 'Create ' . ucwords($menu) . ' Contents')
    @endif
    @section('styles')
        {{-- tinymce --}}
    ...
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            @if (isset($is_SubMenu))
                {{ __('Create content for ') . ucwords($subMenu) }}
            @else
                {{ __('Create content for ') . ucwords($menu) }}
            @endif
        </h2>
    </x-slot>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class=" overflow-hidden shadow-sm sm:rounded-lg">
                <div class=" text-gray-900 p-6 bg-white">
                    <form action="{{ route('user.contents-store') }}" method="post" id="formContent">
